...
def merge_various_variables_mask(mask_array, mask_cut, state, out_path, out_file, lat1, lat2, lon1, lon2, area_name,
                                 doy, n_hist, ignore_fields=list(), sentinel_path=None, sentinel_9km_path=None):
    ...
    if sentinel_9km_path is not None:
        fh_dic["sentinel_9km"] = Dataset(os.path.join(sentinel_9km_path, doy + ".nc"), "r")
    ...
    if mask_cut:
        mask_array = mask_array[i_lat_start: i_lat_end + 1, i_lon_start: i_lon_end + 1]
    ma_dic = {"provided": mask_array}
    fh_states = Dataset(os.path.join("Data", "US_States", "usa_states.nc"), "r")
    states_array = fh_states.variables["states_flag"][:]
    state_mask = ma.getmaskarray(ma.masked_where(states_array != states_index_dic[state], states_array))
    ma_dic["state"] = state_mask[i_lat_start: i_lat_end + 1, i_lon_start: i_lon_end + 1]
    ...
    print("Before mask, number of valid grids:", fh_out.variables["smap_p_e_soil_moisture"][:].count())
    # for var in fh_out.variables:
    #     if var != "lat" and var != "lon":
    #         if ma.is_masked(fh_out.variables[var][:]):
    #             print(var, ma.array(fh_out.variables["smap_p_e_soil_moisture"][:], mask=
    #             ma.mask_or(ma.getmaskarray(fh_out.variables["smap_p_e_soil_moisture"][:]),
    ...
    landcover_values = fh_dic["sentinel_landcover"].variables["landcover_class"][i_lat_start: i_lat_end + 1,
    ...
    print("After mask, number of valid grids:", fh_out.variables["smap_p_e_soil_moisture"][:].count())


def merge_various_variables_mask_v2(mask_array, mask_cut, out_path, out_file, lat1, lat2, lon1, lon2, area_name,
                                    doy, n_hist, selected_sentinel_fields, sentinel_path=None, state_mask=False, state=None):
    print(area_name, doy, out_file)
    fh_dic = dict()
    rb_hist = "sentinel_rb_hist_" + str(n_hist)
    fh_dic[rb_hist] = Dataset(os.path.join("Data", "Sentinel", "usa_rb_hist_average_" + str(n_hist), doy + ".nc"), "r")
    ...
    all_mask_array = []
    for arr in ma_dic.values():
        all_mask_array.append(arr)
    daily_mask = np.logical_or.reduce(all_mask_array)
    print("Before mask, number of valid grids:", fh_out.variables["smap_p_e_soil_moisture"][:].count())
    # for var in fh_out.variables:
    #     if var != "lat" and var != "lon":
    #         if ma.is_masked(fh_out.variables[var][:]):
...
def merge_various_variables_mask_usa(mask_array, out_path, out_file, doy, n_hist, ignore_fields=list(),
                                     sentinel_path=None, sentinel_9km_path=None):
    print(doy, out_file)
    fh_dic = dict()
    fh_dic["sentinel_landcover"] = Dataset(os.path.join("Data", "Sentinel", "landcover_class_usa.nc"), "r")
    fh_dic["sentinel_surface_flags"] = Dataset(os.path.join("Data", "Sentinel", "static_surface_flags_usa.nc"), "r")
    ...
    dim_dic = {}
    for name, dim in fh_dic["sentinel_landcover"].dimensions.items():
        fh_out.createDimension(name, len(dim))
    for v_name, varin in fh_dic["sentinel_landcover"].variables.items():
