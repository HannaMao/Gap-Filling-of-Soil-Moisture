...
from soil_moisture_downscaling.utils import find_index, get_lat_lon


def _out2NC(path, f_reference, fName):
    ...
    for index, row in df.iterrows():
        ...
        for var in list(set(var_list) - {"lat", "lon"}):
            if var not in fh.variables:
                fh.createVariable(var, "f4", ("lat", "lon"))
...
def convert2nc(path, f_reference, fName=None):
    if fName:
        _out2NC(path, f_reference, fName + ".csv")
    else:
        for csv_file in os.listdir(path):
            if csv_file.endswith('.csv'):
                _out2NC(path, f_reference, csv_file)
